...
RSpec.describe InvitationsController, :type => :controller do
  ...
  describe "it handles the token" do
    ...
    it 'checks token' do
      get :activate,  params: {:token => @invitation.token}
  ...
  describe "creates a user" do
    it 'verifies new user if invitee is an admin' do
      all = User.count
      post :sign_up,  params: {:token => @invitation.token,:user => { email: "test@aol.com",password: 'blue32blue32', password_confirmation: "blue32blue32", name: "test", legacy_organization_id:@invitation.organization_id, referring_user_id:@invitation.user_id, accepted_terms: true }}
      expect(User.count).to eq(all + 1)
    end

    it 'verifies new user if invitee is an admin' do
      all = User.count
      post :sign_up, params: { :token => @dhruv_invitation.token,:user => { email: "test@aol.com",password: 'blue32blue32', password_confirmation: "blue32blue32", name: "test", legacy_organization_id:@invitation.organization_id, referring_user_id:@invitation.user_id, accepted_terms: true }}
      expect(User.count).to eq(all + 1)
    end
  end
end
