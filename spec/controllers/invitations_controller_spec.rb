...
RSpec.describe InvitationsController, :type => :controller do
  ...
  before do |example|
    ...
    @frank = User.create(name:'Frank', email:'Frank@aol.com', password:'blue32blue32', legacy_organization_id: org.id, admin:true) 
    @dhruv = User.create(name:'Dhruv', email:'Dhruv@gmail.com', password:'blue32blue32', legacy_organization_id: org.id, admin:false) 
  ...
  describe "creates a user" do 
    it 'verifies new user if invitee is an admin' do  
      all = User.count
      post :sign_up,  :token => @invitation.token,:user => { email: "test@aol.com",password: 'blue32blue32',name: "test", legacy_organization_id:@invitation.organization_id, referring_user_id:@invitation.user_id }
      expect(User.count).to eq(all + 1)
      expect(User.last.verified).to eq(true)
    end
    it 'verifies new user if invitee is an admin' do  
      all = User.count
      post :sign_up,  :token => @dhruv_invitation.token,:user => { email: "test@aol.com",password: 'blue32blue32',name: "test", legacy_organization_id:@invitation.organization_id, referring_user_id:@invitation.user_id }
      expect(User.count).to eq(all + 1)
