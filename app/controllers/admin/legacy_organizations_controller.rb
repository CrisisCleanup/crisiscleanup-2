  module Admin
    ...
    def index
      @orgs = Legacy::LegacyOrganization.order("name").paginate(:page => params[:page], :per_page => 750)
      ...
      max_updated_at = @orgs.maximum(:updated_at).try(:utc).try(:to_s, :number)
      @cache_key = "org-table-#{@orgs.count}-#{max_updated_at}"
    end
