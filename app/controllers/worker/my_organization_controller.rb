module Worker
  class MyOrganizationController < ApplicationController
    ...
    def index
      @pending_invitations  = Invitation.select("invitations.*, users.name as user_name")
                                  .joins("JOIN users ON users.id = invitations.user_id")
        .where(organization_id: current_user.legacy_organization_id, activated: false).order(:invitee_email).paginate(:page => params[:pending_invite_page])
